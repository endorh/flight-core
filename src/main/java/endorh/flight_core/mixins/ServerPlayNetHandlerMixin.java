...
/**
 * Redirects checks to {@link GameRules#DISABLE_ELYTRA_MOVEMENT_CHECK} to
...
public abstract class ServerPlayNetHandlerMixin {
	 ...
	 * {@link GameRules#DISABLE_ELYTRA_MOVEMENT_CHECK} when applying the
	 * speed check at {@link ServerPlayNetHandler#processPlayer} to
	 ...
	 * @param key Key for the {@link GameRules#DISABLE_ELYTRA_MOVEMENT_CHECK} rule.
	...
	public boolean _flightcore_shouldDisableElytraCheck(
	  GameRules rules, RuleKey<BooleanValue> key, CPlayerPacket packet
		...
		if (key != GameRules.DISABLE_ELYTRA_MOVEMENT_CHECK) {
			// Log a warning once
		...
		if (!player.isElytraFlying())
			return false;
		// Mimic the check
		double pX = packet.getX(player.getPosX());
		double pY = packet.getY(player.getPosY());
		double pZ = packet.getZ(player.getPosZ());
		double dX = pX - firstGoodX;
		double dY = pY - firstGoodY;
		double dZ = pZ - firstGoodZ;
		double playerMotion2 = player.getMotion().lengthSquared();
